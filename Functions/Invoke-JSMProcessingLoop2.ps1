function Invoke-JSMProcessingLoop
{
    ...
    Initialize-TrackingVariable #Creates the variables used throughout the loop
    ##################################################################
    #Get the Required Jobs from the JobDefinitions and add them to JobRequired
    ##################################################################
    try
    {
        ...
        @(Get-JSMJobRequired @GRJParams).ForEach({$JobRequired.add($_)})
        Write-Verbose -Message $message
    }
    catch
    {
