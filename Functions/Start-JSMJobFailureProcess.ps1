Function Start-JSMJobFailureProcess
{
    ...
    foreach ($j in $NewJobFailure)
    {
        ...
        if ($JobFailure.$($j.name).FailureCount -ge $JobFailureRetryLimitForThisJob)
        {
            $message = "Exceeded JobFailureRetry Limit. Ending Job Processing Loop. Failure Count: $($JobFailure.$($j.name).FailureCount). FailureTypes: $($JobFailure.$($j.name).FailureType -join ',')"
        ...
        {
            $message ="$($j.Name): JobFailureRetry Limit Not Exceeded. Failure Count: $($JobFailure.$($j.name).FailureCount). FailureTypes: $($JobFailure.$($j.name).FailureType -join ',')"
