...
InModuleScope JobSetManager {
    Describe "Add-JSMProcessingStatusEntry Unit Tests" -Tag 'UnitTests' {
        Context "Validate parameters" {
            $defaultParamCount = 11
            [object[]]$params = (Get-ChildItem "function:\Add-JSMProcessingStatusEntry").Parameters.Keys
...
InModuleScope JobSetManager {
    Describe "Add-JSMProcessingStatusEntry Integration Tests" -Tags "IntegrationTests" {
...
InModuleScope JobSetManager {
    Describe "Add-JSMProcessingStatusEntry Example Tests" -Tags "Example" {
        Context "Adds the Correct Entry" {
...
Remove-Module -Name JobSetManager -Force